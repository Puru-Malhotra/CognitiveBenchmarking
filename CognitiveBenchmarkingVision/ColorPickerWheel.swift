...
//  CognitiveBenchmarkingVision
//
//  Created by Sankaet Cheemalamarri on 10/27/24.
//
...
struct CustomColorPickerWheel: View {
    ...
    var body: some View {
        ...
        .onChange(of: saturation) { newValue in
            bgColor = calculateColor()
            print(knobLocation, center, radius, saturation)
            moduleState.path.append(PassthroughModuleState.PathData(location: knobLocation!,
                                 center: center!,
                                 radius: radius,
                                 saturation: saturation))
        }
